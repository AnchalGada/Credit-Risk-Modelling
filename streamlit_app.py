...
Exact mirror of Credit_Risk_Model.ipynb with corrected synthetic data
that has realistic correlations so inputs change the prediction.
"""
import warnings
...
# ─────────────────────────────────────────────────────────────
# SYNTHETIC DATA WITH REALISTIC CORRELATIONS
# The key fix: Bad_label is driven by actual risk features
# so the model learns real patterns and inputs change predictions
# ─────────────────────────────────────────────────────────────
def generate_synthetic_data():
    ...
    # ── Step 1: Generate raw risk features per customer ──────
    util_ratio   = np.random.beta(2, 5, N)          # 0-1, mostly low
    dpd30_rate   = np.random.beta(1, 9, N)           # 0-1, mostly low
    ontime_rate  = 1 - dpd30_rate * 0.8 + np.random.normal(0, 0.05, N)
    ontime_rate  = np.clip(ontime_rate, 0, 1)
    past_due_r   = np.random.exponential(0.05, N).clip(0, 1)
    enq_90       = np.random.poisson(0.8, N).clip(0, 10)
    months_age   = np.random.randint(6, 180, N)

    # ── Step 2: Bad_label driven by risk features (realistic) 
    log_odds = (
        -3.0
        + 4.5  * dpd30_rate       # 30+ DPD is the strongest predictor
        - 3.0  * ontime_rate       # on-time payment reduces risk
        + 3.0  * util_ratio        # high utilisation = high risk
        + 4.0  * past_due_r        # past due = very high risk
        + 0.8  * (enq_90 / 10)    # many enquiries = higher risk
        - 0.5  * (months_age / 180) # older account = lower risk
    )
    prob_bad = 1 / (1 + np.exp(-log_odds))
    bad_label = np.random.binomial(1, prob_bad)

    # ── Step 3: Demographics features (feature_1..79) ────────
    df_demographics = pd.DataFrame({
        "customer_no": customer_nos,
        **{f"feature_{i}": np.random.randn(N) for i in range(1, 80)},
        "Bad_label": bad_label
    })
    # feature_1..5 correlated with bad_label (same as notebook)
    for c in ["feature_1", "feature_2", "feature_3", "feature_4", "feature_5"]:
        df_demographics.loc[bad_label == 1, c] += 1.2

    # ── Step 4: Account table — correlated with bad_label ────
    n_acc = N * 3
    ...
    cust_pdue  = dict(zip(customer_nos, past_due_r))

    credit_lims = np.random.uniform(20000, 1000000, n_acc)
    util_vals   = np.array([cust_util.get(c, 0.3) for c in ac])
    bal_vals    = (util_vals * credit_lims).clip(0, 300000)
    dpd_vals    = np.array([cust_dpd.get(c, 0.05) for c in ac])
    ok_vals     = np.array([cust_ok.get(c, 0.9) for c in ac])
    pdue_vals   = np.array([cust_pdue.get(c, 0.02) for c in ac])
    past_due_amt = (pdue_vals * bal_vals).clip(0, 50000)

    # Build payment history strings correlated with dpd/ok rates
    ...
    ph_strings = [make_ph(dpd_vals[i], ok_vals[i]) for i in range(n_acc)]
...
# ─────────────────────────────────────────────────────────────
# FEATURE ENGINEERING — exact copy from notebook Cell 11
# ─────────────────────────────────────────────────────────────
def parse_ph(s):
...
# ─────────────────────────────────────────────────────────────
# TRAIN ALL 5 MODELS — exact same as notebook
# ─────────────────────────────────────────────────────────────
...
def train_all_models():
    ...
    # Ensemble — exact same as notebook Cell 25
    ...
    # Pick best by Gini — same as notebook Cell 29
    all_results = [
...
# ─────────────────────────────────────────────────────────────
# PREDICT — exact same as notebook Cell 35
# ─────────────────────────────────────────────────────────────
def predict_customer(state, cur_balance, credit_limit, cash_limit, past_due,
    ...
    # Exact same input_dict as notebook Cell 35
    input_dict = {
    ...
    row_input = scaler.transform(row_imp) if needs_scaling else row_imp.values

    prob_bad     = float(best_model.predict_proba(row_input)[0, 1])
    label        = int(prob_bad >= 0.5)
    credit_score = int(300 + (1 - prob_bad) * 550)  # exact notebook formula
    ...
    return {"bad_label": label, "prob_bad": round(prob_bad, 4),
...
# ── Load model ────────────────────────────────────────────────
with st.spinner("🔄 Training all 5 models (LR, RF, XGBoost, LightGBM, Ensemble) — takes ~2 min on first load..."):
...
if page == "Single Customer":
    ...
    with st.form("customer_form"):
        ...
        st.markdown('<p class="section-header">Payment History</p>',
                    unsafe_allow_html=True)
        c7, c8 = st.columns(2)
        with c7:
            ontime_rate = st.slider(
                "On-time Payment Rate",  0.0, 1.0, 0.95, 0.01,
                help="1.0 = always paid on time  |  0.0 = never paid on time"
            )
        with c8:
            dpd30_rate  = st.slider(
                "30+ DPD Rate",          0.0, 1.0, 0.00, 0.01,
                help="Fraction of payments that were 30+ days overdue"
            )
        ...
        unsecured = st.slider(
            "Unsecured Enquiry Ratio",  0.0, 1.0, 0.20, 0.01,
            help="0 = all secured (HL/AL loans)  |  1 = all unsecured"
        )
    ...
    if submitted:
        ...
        m1.markdown(f'<div class="metric-card"><h3>Default Probability</h3>'
                    f'<p>{prob:.1f}%</p></div>', unsafe_allow_html=True)
        m2.markdown(f'<div class="metric-card"><h3>Safe Probability</h3>'
                    f'<p>{result["prob_good"]*100:.1f}%</p></div>', unsafe_allow_html=True)
        m3.markdown(f'<div class="metric-card"><h3>Risk Band</h3>'
        ...
        with col2:
            ...
            st.metric("Recent Enquiries (90d)", str(enq_90),
...
# ═══════════════════════════════════════════════════════════════
    ...
    <div class="hero">
        <h1>📂 Batch Scoring</h1>
        <p>Bank GoodCredit · Upload a CSV of customers and download results</p>
    </div>""", unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════════
    ...
    <div class="hero">
        ...
        <p>Bank GoodCredit · All 5 models trained same as Jupyter Notebook</p>
    </div>""", unsafe_allow_html=True)

    st.markdown("### All 5 Models — Gini Scores")
    rows = []
    ...
    df_s = pd.DataFrame(rows).sort_values("Gini", ascending=False).reset_index(drop=True)
    st.dataframe(df_s, use_container_width=True, hide_index=True)

    st.markdown(f"### 🏆 Best Model: `{state['best_name']}`")
    c1, c2, c3 = st.columns(3)
    c1.markdown(f'<div class="metric-card"><h3>Gini Score</h3>'
                f'<p>{state["best_gini"]:.2f}</p></div>', unsafe_allow_html=True)
    c2.markdown(f'<div class="metric-card"><h3>AUC Score</h3>'
                f'<p>{state["best_auc"]:.4f}</p></div>', unsafe_allow_html=True)
    c3.markdown(f'<div class="metric-card"><h3>Benchmark Gini</h3>'
    ...
    st.markdown("### Credit Score Formula (same as notebook Cell 35)")
    st.code("credit_score = 300 + (1 - prob_bad) × 550", language="python")

    st.markdown("### Top Features from Problem Statement (PM-PR-0015)")
    st.table(pd.DataFrame({
        "Feature": [
